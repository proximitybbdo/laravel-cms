    ...
    <ul class="nav nav-tabs">
        @if($custom_view != 'bbdocms::admin.partials.form')
            @foreach($languages as $admin_lang)
                ...
                ?>
                <li class="{{ $admin_lang['short'] == $lang ? 'active':'' }}"><a class="lang_content"
                                                                                 href="{{ $url }}">{{ $admin_lang['short'] }}
                        <span class="label
      label-{{ ($status == 'online' ?'success' : ($status == 'edit' ? 'warning' : 'primary')) }}">{{ $status }}</span></a>
                </li>
            @endforeach
        @else
            <?php $default_lang = config('cms.default_locale'); ?>
            <li class="active"><a class="lang_content" href="#">{{ $default_lang }} <span
                            class="label label-primary"></span></a></li>
        @endif
    </ul>
</div>
