...
@if($block_list != null)
        ...
        <div class="input-group">
            <select class="form-control col-sm-10" id="ddl_add_block">
                @foreach($block_list as $block_type)
                    <option value="{{ $block_type['type'] }}"
                            data-amount="{{ $block_type['amount'] }}">{{ $block_type['description'] }}</option>
                @endforeach
            </select>

            <span class="input-group-btn">
                <button id="btn_add_block" class="btn btn-primary" data-version="{{ $version }}"
                        data-id="{{ $model->id }}" data-lang="{{ $lang }}"
                        data-action="{{ $action }}">Add Block</button>
            </span>
        </div>
    </div>
    @endif
    ...
    <!--Content type item fields-->
        @foreach( config('cms.'.strtoupper($module_type).'.fields') as $field_arr )
            @if( in_array($field_arr['form'], ['select','file','checkbox','radio']) )
