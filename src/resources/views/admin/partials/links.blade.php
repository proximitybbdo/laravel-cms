            ...
            <div class="row">
                ...
                <ul class="col-sm-12 selected-links" id="selected-{{ $link['field'] }}">
                    <?php foreach($link['items'] as $item): ?>
                          <?php 
                            ...
                            ?>
                          <li id="li-link-{{$key}}-{{ $item->id }}" class="label label-primary"  <?= $checked == true ? '' : "style=display:none" ?>>                   
                            {{ $item->description }}
              ...
              <div class="col-sm-4">  
                <button class="link-add-button btn btn-success" 
                  ...
                  Add {{ Config::get('admin.' . $key . '.description') }}
                </button>
              </div>
            </div>
          <?php else: ?>
            <div class="form-group">
            <select id="{{ $link['group_field'] }}" class="form-control links chosen_links" name="{{ $link['group_field'] }}" multiple>
              <?php foreach($link['items'] as $item): ?>
                <option value="<?= $item->id ?>" <?= $item->item_id != null ? 'selected' : '' ?>><?= $item->description ?></option>
