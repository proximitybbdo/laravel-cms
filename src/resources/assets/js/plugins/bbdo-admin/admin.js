$(document).ready(function () {
    ...
    $("body .modal").on("submit", "form", function (e) {
        ...
        if (module_type) {
            $.ajax({
                type: "POST",
                url:
                    window.base_url + "/icontrol/items/" + module_type + "/post/" + lang,
                data: $(this).serialize(),
                success: function (data) {
    ...
    function initSortable() {
        ...
        if ($("#posts.sortable tbody").length) {
        ...
        if ($("#blocks").length) {
            $("#blocks").sortable({
                placeholder: "block block-rounded block-bordered blockcontent_block",
                helper: "clone"
            });
        }
    }
    ...
    function setInputStatus() {
        $("input[type=checkbox]").each(function () {
            if (this.name.indexOf("_enabled") > 0) {
                $('input[name="prop[' + this.name.replace("_enabled", "") + ']"]').prop(
                    "disabled",
                    !this.checked
                );
            }
    ...
    $(document).on("click", "#btn_add_block", function (event) {
        ...
        $.ajax({
            type: "POST",
            url: window.base_url + "/icontrol/items/" + module_type + "/renderblock",
            data: {
                type: type,
    ...
    $(document).on("click", ".multiple_control_add", function () {
        var amount_visible = $(this)
            ...
            .find(".form-group:visible").length;
        $(this)
            .closest(".multiple_container")
            .find(".form-group:eq(" + amount_visible + ")")
            .show();
